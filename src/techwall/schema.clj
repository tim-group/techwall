...
(def ^{:private true} migrations
  ["CREATE TABLE MIGRATIONS (ID INT PRIMARY KEY, EXECUTION_DATETIME TIMESTAMP)"
   ...
   "CREATE TABLE VERSIONS (ID INT PRIMARY KEY AUTO_INCREMENT, CREATION_DATETIME TIMESTAMP)"
   "CREATE TABLE ALLOCATIONS (ID INT PRIMARY KEY AUTO_INCREMENT, VERSION_ID INT, WALL_ID INT, CATEGORY_ID INT, TECHNOLOGY_ID INT)"
   ])
